...
{
    demo_input_databuf_t *db  = (demo_input_databuf_t *)args->obuf;
    ...
    while (run_threads()) {
         ...
         */
        while ((rv=demo_input_databuf_wait_free(db, block_idx)) 
                != HASHPIPE_OK) {
	...
	db->block[block_idx].number1=a;
        ...
        // Mark block as full
        demo_input_databuf_set_filled(db, block_idx);
...
static hashpipe_thread_desc_t fake_net_thread = {
    name: "demo_fake_net_thread",
    skey: "NETSTAT",
    init: NULL,
    run:  run,
    ibuf_desc: {NULL},
    obuf_desc: {demo_input_databuf_create}
};
